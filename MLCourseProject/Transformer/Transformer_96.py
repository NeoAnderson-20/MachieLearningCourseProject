...
# 0. 参数配置
'''
最优参数设置：batch_size = 128; epoches = 300; learning_rate = 3e-5;
或者：batch_size = 32; epoches = 300; learning_rate = 3e-5
'''
class Config():
    data_path = './ETTh1.csv'
    ...
    output_size = 96  # 由于是单输出任务，最终输出层大小为1，预测未来1天风速,相当于输出时间步
...
class Transformer(nn.Module):
    ...
    def __init__(self, n_encoder_inputs, n_decoder_inputs, Sequence_length, d_model=512, dropout=0.1, num_layer=8):
        ...
        self.ziji_add_linear = torch.nn.Linear(Sequence_length, 96* 7)
    ...
    def forward(self, src, target_in):
        ...
        out = out.view(config.batch_size, 96, 7)
        return out

# 11. 创建Transformer 模型，设置损失函数和优化器
model = Transformer(n_encoder_inputs=7, n_decoder_inputs=7, Sequence_length=96).to(device)  # 96 表示Sequence_length  transformer 输入数据 序列的长度
...
for epoch in range(config.epochs):
    ...
    for data in train_bar:
        ...
        # print(y_train.shape)
        optimizer.zero_grad()
        tgt_in = torch.rand((config.batch_size, 96, 7)).to(device)
...
for data in train_bar:
    ...
    my_list1.extend(
        scaler_train.inverse_transform((y_train_pred.detach()).numpy()[96])[:, -1].reshape(-1, 1).flatten())
    plt.plot(range(96, 192), my_list1, "b", label='Prediction')

    my_list2.extend(
        scaler_train.inverse_transform((y_train_tensor.detach()).numpy()[0])[:, -1].reshape(-1, 1).flatten())
    my_list2.extend(
        scaler_train.inverse_transform((y_train_tensor.detach()).numpy()[96])[:, -1].reshape(-1, 1).flatten())
...
for data in test_bar:
    ...
    my_list3.extend(scaler_test.inverse_transform((y_test_pred.detach()).numpy()[96])[:, -1].reshape(-1, 1).flatten())
    plt.plot(range(96, 192), my_list3, "b", label='Prediction')
    print(my_list3)
    my_list4.extend(scaler_test.inverse_transform((y_test_tensor.detach()).numpy()[0])[:, -1].reshape(-1, 1).flatten())
    my_list4.extend(scaler_test.inverse_transform((y_test_tensor.detach()).numpy()[96])[:, -1].reshape(-1, 1).flatten())
    my_list5 = scaler_test.inverse_transform((y_test_tensor.detach()).numpy()[96])[:, -1].reshape(-1, 1).flatten()

    print(scaler_test.inverse_transform((y_test_tensor.detach()).numpy()[96])[:, -1].reshape(-1, 1).flatten())
